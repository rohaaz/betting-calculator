...
export default function BettingCalculator() {
 ...
 // Calculate percentage return for Back/Lay
 const percentReturn = (backStake && (profitIfBack !== 0 || profitIfLay !== 0)) 
   ? (Math.min(profitIfBack, profitIfLay) / parseFloat(backStake)) * 100 
   : 0;

 // Calculate percentage return for Dutch betting
 const calculateDutchPercentReturn = () => {
   const totalStake = parseFloat(homeStake || 0) + parseFloat(awayStake || 0);
   if (!totalStake) return 0;
   
   // Account for SNR bets by only counting actual money invested
   const totalActualStake = (homeSNR ? 0 : parseFloat(homeStake || 0)) + 
                          (awaySNR ? 0 : parseFloat(awayStake || 0));
   
   if (totalActualStake === 0) return 0; // Avoid division by zero
   
   // Use the minimum profit across all outcomes
   const minProfit = Math.min(
     calculateDutchProfit("home"),
     calculateDutchProfit("away")
   );
   
   return (minProfit / totalActualStake) * 100;
 };

 // Calculate percentage return for Three-Way Dutch
 const calculateThreeWayPercentReturn = () => {
   const totalStake = parseFloat(homeStake || 0) + parseFloat(drawStake || 0) + parseFloat(awayStake || 0);
   if (!totalStake) return 0;
   
   // Use the minimum profit across all outcomes
   const minProfit = Math.min(
     calculateThreeWayProfit("home"),
     calculateThreeWayProfit("draw"),
     calculateThreeWayProfit("away")
   );
   
   return (minProfit / totalStake) * 100;
 };
       ...
       {activeTab === "dutch" && (
         <div style={styles.card}>
           ...
           <div style={styles.summary}>
             ...
             {/* Only show percentage return if no SNR checkboxes are checked */}
             {!homeSNR && !awaySNR && (
               <p><strong>Percentage Return:</strong> 
                 <span style={calculateDutchPercentReturn() >= 0 ? styles.profit : styles.loss}>
                   {" "}{calculateDutchPercentReturn().toFixed(2)}%
                 </span>
               </p>
             )}
       ...
       {activeTab === "threeWay" && (
         <div style={styles.card}>
           ...
           <div style={styles.summary}>
             ...
             <p><strong>Percentage Return:</strong> 
               <span style={calculateThreeWayPercentReturn() >= 0 ? styles.profit : styles.loss}>
                 {" "}{calculateThreeWayPercentReturn().toFixed(2)}%
               </span>
             </p>
             <p style={{marginTop: "10px"}}><strong>Place these bets to lock in profit:</strong></p>
             <p style={styles.profit}>${parseFloat(homeStake || 0).toFixed(2)} on Home at the best available odds.</p>
